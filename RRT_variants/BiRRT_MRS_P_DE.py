...
def update_extension_distance(d, exponent, max_d, min_d):
    if exponent < 0:
        new_d = d / np.power(2, abs(exponent))
    else:
        new_d = d * np.power(2, exponent)
    if new_d > max_d:
        exponent = round(math.log2(max_d/d))
        new_d = max_d
    elif new_d < min_d:
        exponent = round(math.log2(min_d/d))
        new_d = min_d
    else:
        exponent = exponent
    return new_d, exponent


def de_p_mrs_bidirectional_rrt_path(obstacles, xy_start, xy_goal, params, priorities):
    NoP = len(xy_start)
    start_trees = []
    goal_trees = []
    ...
    limit1 = 10
    limit2 = 10
    collision_occurred_start = 0
    collision_occurred_goal = 0
    num_of_consecutive_valid_samples_start = 0
    num_of_consecutive_valid_samples_goal = 0
    exponent_start = 0
    exponent_goal = 0
    ...
    while np.sum(it_start_tree) + np.sum(it_goal_tree) < max_it:
        ...
        if not all_trees_connected:
            ...
            start_tree_d = start_tree.extension_d
            goal_tree_d = goal_tree.extension_d
            ...
            new_node_p = closest_node.p + start_tree_d * (q_samp - closest_node.p) / np.linalg.norm(q_samp - closest_node.p)
            if not EdgeIsCollisionFree(obstacles, closest_node.p, new_node_p):
                it_start_tree[active_tree_id] += 1
                num_of_consecutive_valid_samples_start = 0
                collision_occurred_start += 1
                if collision_occurred_start >= limit1:
                    collision_occurred_start = 0
                    exponent_start -= 1
                    start_tree.extension_d, exponent_start = update_extension_distance(d, exponent_start, d_max, d_min)
                continue
            else:
                collision_occurred_start = 0
                num_of_consecutive_valid_samples_start += 1
                if num_of_consecutive_valid_samples_start >= limit2:
                    num_of_consecutive_valid_samples_start = 0
                    exponent_start += 1
                    start_tree.extension_d, exponent_start = update_extension_distance(d, exponent_start, d_max, d_min)
            ...
            # Extend the tree towards the sample
            new_node_p = closest_node.p + goal_tree_d * (q_samp - closest_node.p) / np.linalg.norm(q_samp - closest_node.p)
            if not EdgeIsCollisionFree(obstacles, closest_node.p, new_node_p):
                it_goal_tree[active_tree_id] += 1
                num_of_consecutive_valid_samples_goal = 0
                collision_occurred_goal += 1
                if collision_occurred_goal >= limit1:
                    collision_occurred_goal = 0
                    exponent_goal -= 1
                    goal_tree.extension_d, exponent_goal = update_extension_distance(d, exponent_goal, d_max, d_min)
                continue
            else:
                collision_occurred_goal = 0
                num_of_consecutive_valid_samples_goal += 1
                if num_of_consecutive_valid_samples_goal >= limit2:
                    num_of_consecutive_valid_samples_goal = 0
                    exponent_goal += 1
                    goal_tree.extension_d, exponent_goal = update_extension_distance(d, exponent_goal, d_max, d_min)
